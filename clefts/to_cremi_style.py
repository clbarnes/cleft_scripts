...
class AbstractCremiFactory(metaclass=ABCMeta):
    ...
    def _populate_raw(
        self, padded_offset_from_stack_px, padded_shape_px, skip_if_exists
    ):
        """
    ...
    def _populate_annotations(
        ...
        with self:
            ...
            ds = f.create_dataset(
                "/annotations/presynaptic_site/pre_to_conn", data=pre_to_conn_arr
            )
...
class CremiFactoryFromN5(AbstractCremiFactory):
    ...
    def set_input_stack(self, n5_ds, resolution_nm_zyx=None, translation_nm_zyx=None):
        ...
        with self:
            self._cremi_file.h5file.attrs[
                "stack_offset_from_project"
            ] = self.translation
...
class CremiFactoryFromCatmaid(AbstractCremiFactory):
    ...
    def set_input_stack(
        self, image_fetcher, resolution_nm_zyx=None, translation_nm_zyx=None
    ):
    ...
    def get_raw(self, offset_px, shape_px):
        roi = np.array(
            [offset_px.to_list("zyx"), (offset_px + shape_px).to_list("zyx")]
        )
        return self.im_fetcher.get_stack_space(roi)


def make_data_from_n5(
    n5_path,
    ds_name,
    output_path,
    offset_px,
    ...
    factory.populate(
        offset_px, shape_px, padding_low_px, padding_high_px, skip_if_exists=True
    )
...
PostAnnotationArgs = namedtuple(
    "PostAnnotationArgs", ["add_annotation", "add_comment", "set_pre_post_partners"]
)


def catmaid_to_annotations_tn_to_tn(
    project_offset_nm, shape_nm, padding_low_nm, padding_high_nm=None, comment=True
):
    """
    ...
    df = catmaid.get_connector_partners(
        set(cid for cid, _ in all_cache.in_box(project_offset_nm, shape_nm))
    )
    ...
    def in_roi(coord):
        ...
        return all(
            upper >= item >= 0
            for item, upper in zip(coord.to_list(), central_roi_shape_nm.to_list())
        )
    ...
    for conn_id, pre_post in sorted(partners.items(), key=lambda x: x[0]):
        if not in_roi(pre_post["pre"]["coord"]) and not any(
            in_roi(d["coord"]) for d in pre_post["post"]
        ):
            continue
...
def catmaid_to_annotations_conn(project_offset_nm, shape_nm, padding_nm, comment=True):
    ...
    for _, row in df.iterrows():
        ...
        annotations.add_annotation(
            cid, "presynaptic_site", [row["connector_" + dim] for dim in "zyx"]
        )
        done.add(cid)
...
def catmaid_to_annotations_conn_to_tn(
    project_offset_nm, shape_nm, padding_nm, comment=True
):
    """
