...
neurocean_creds = json.loads(
    (Path.home() / ".secrets" / "catmaid" / "neurocean.json").read_text()
)
...
volume = n5.create_dataset(
    ...
    shape=[roi["stack"]["shape"][dim] + 2 * PADDING_PX[dim] for dim in DIMS],
    compression="blosc",
    chunks=(32, 256, 256),
)
...
volume.attrs["offset_from_stack_origin_px"] = {
    dim: roi["stack"]["offset"][dim] - PADDING_PX[dim] for dim in DIMS
}
...
volume.attrs[
    "description"
] = "Block of the {}. antennal lobe of L1 larva containing ~1000 synapses".format(SIDE)
