...
from clefts.constants import (
    DIMS, STACK_ID,
    CREDENTIALS_PATH, ANTENNAL_LOBE_OUTPUT, OUTPUT_ROOT, )
...
def get_antennal_lobe_centers(force=False):
    connectors_df = get_antennal_lobe_df(ANTENNAL_LOBE_OUTPUT / 'all_ORN_connectors.csv', force)

    centers = dict()
    for side in 'lr':
        subset = connectors_df.loc[connectors_df['side'] == side]
        centers[side] = dict(zip('zyx', subset[['zp', 'yp', 'xp']].mean(axis=0).squeeze()))
...
def broken_slices_in_roi(offset_p, shape_p):
    ...
    for z_s in im_fetcher.stack.broken_slices:
        ...
        if offset_p[depth_dim] <= proj_coord <= offset_p[depth_dim] + shape_p[depth_dim]:
            counter += 1
...
def get_connectors_in_cube(offset_p, shape_p, limit=1_000_000):
    ...
    if broken_count:
        raise BrokenSliceError('This ROI contains {} broken slices'.format(broken_count))
    ...
    while True:
        ...
        while True:
            try:
                ...
                logging.debug('Max volume of %snm^3 was too large, trying %snm^3', max_volume, new_max_vol)
        ...
        if count < min_syn:
            ...
            logger.debug('Attempt %s with radius %snm failed, trying %snm', counter, radius_nm, new_radius_nm)
...
def main_by_catmaid_api():
    ...
    if json_path.is_file() and conns_path.is_file:
        ...
        roi_str = json.dumps({'offset': offset_p, 'shape': shape_p, 'space': 'project'}, sort_keys=True, indent=2)
...
def main():
    ...
    d = {
        ...
        'stack': {
            'offset': {dim: int(val) for dim, val in coord_trans.project_to_stack(offset_p).items()},
            'shape': {dim: ceil(val / coord_trans.resolution[dim]) for dim, val in shape_p.items()},
        },
