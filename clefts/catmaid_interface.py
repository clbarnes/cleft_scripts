...
def subdivide_roi(offset, shape, max_volume):
    ...
    if vol < max_volume:
        ...
        new_shape = {key: val/2 if key == split_dim else val for key, val in shape.items()}
        yield from subdivide_roi(offset, new_shape, max_volume)
        new_offset = {key: val + new_shape[key] if key == split_dim else val for key, val in offset.items()}
...
class CircuitConnectorAPI(CatmaidClientApplication):
    ...
    def get_annotation_id(self, annotation_id_or_name):
        ...
        annotation_ids = [d['id'] for d in all_annotations if d['name'] == annotation_id_or_name]
    ...
    def get_skeletons_by_annotation(self, annotation_id_or_name):
        ...
        response = self.post((self.project_id, 'annotations', 'query-targets'), data)['entities']
        ...
        for row in response:
            ...
            rows.append({
                ...
                'annotations': {annotation['name']: annotation['id'] for annotation in row['annotations']}
            })
    ...
    def get_synapses_among(self, skeleton_ids):
        ...
        response = self.post((self.project_id, 'connector', 'list', 'many_to_many'), data)
        headers = ['connector_id', 'x', 'y', 'z', 'pre_tnid', 'pre_skid', 'post_tnid', 'post_skid']
    ...
    def get_synapses_between(self, pre_skels, post_skels):
        ...
        for r in response:
            rows.append([
                ...
                r[2], r[3], r[6][0], r[6][1], r[6][2],
                r[7], r[8], r[11][0], r[11][1], r[11][2]
            ])
    ...
    def get_connector_partners_raw_dicts(self, connector_ids):
        data = {
            'cids': list(connector_ids)
        }
        response = self.post((self.project_id, 'connector', 'info'), data)
        response_headers = [
            'connector_id', 'connector_xyz',
            'pre_tnid', 'pre_skid', 'pre_conf', 'pre_uid', 'pre_xyz',
            'post_tnid', 'post_skid', 'post_conf', 'post_uid', 'post_xyz'
        ]
        for row in response:
    ...
    def get_connector_partners(self, connector_ids):
        ...
        df['edge_length'] = np.linalg.norm(df.loc[:, ['edge_' + dim for dim in dims]], axis=1)
        return df
    ...
    def get_skeleton_connectors(self, *skeleton_ids):
        ...
        for relation in ['presynaptic_to', 'postsynaptic_to']:
            params = {'skeleton_ids': list(skeleton_ids), 'with_tags': 'false', 'relation_type': relation}
            ...
            for row in response:
                df_rows.append([
                    row[0], row[1], relation, row[4], row[3], row[2]
                ])
    ...
    def get_connectors_in_roi(self, offset_p, shape_p, max_vol=2_000_000_000):
        ...
        for offset, shape in tqdm(list(subdivide_roi(offset_p, shape_p, max_vol))):
            ...
            treenodes, connectors, _, limit_reached, _ = self.post((self.project_id, 'nodes/'), data)

            if limit_reached:
                raise TooManyNodesError('Node limit reached: use a smaller ROI or max_vol')

            for row in connectors:
                d = dict(zip(response_headers, row))
                if d['id'] in done_ids:
                    continue
                df_rows.append([d['id'], d['location_z'], d['location_y'], d['location_x']])
                done_ids.add(d['id'])
    ...
    def get_skel_names_single(self, *skeleton_ids):
        return {
            int(skid): self.get((self.project_id, "skeleton", int(skid), "neuronname"))["neuronname"]
...
def get_connectors_in_roi(offset_p, shape_p, project_id=PROJECT_ID):
    ...
    query_vars = {
        (dim + end): bounds_dicts[end][dim]
        for dim in 'xyz' for end in ('min', 'max')
    }
