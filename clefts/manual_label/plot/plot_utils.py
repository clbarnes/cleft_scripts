...
from clefts.manual_label.skeleton import Skeleton, Crossing
...
def filter_graph_nodes(g: nx.DiGraph, filter_fn: Callable) -> nx.DiGraph:
    ...
    for node, data in g.nodes(data=True):
        if not filter_fn(data["skeleton"]):
            g2.remove_node(node)
            g2.graph["skeleton"].remove(data["skeleton"])
    return g2
...
def contract_skeletons(g_single: nx.DiGraph, skeleton_groups):
    skid_mapping = dict()
    for group in skeleton_groups:
        for skel in group:
            skid_mapping[skel.id] = group.id
    ...
    for pre_skid, post_skid, data in g_single.edges(data=True):
        ...
        if not (pre_id, post_id) in g_single.edges:
            g_single.add_edge(pre_id, post_id, area=0, count=0, edges=[])

        g.edges[pre_id, post_id]["area"] += data["area"]
        g.edges[pre_id, post_id]["count"] += data["count"]
        g.edges[pre_id, post_id]["edges"].extend(deepcopy(data["edges"]))
