...
from clefts.constants import (
    RESOLUTION, TRANSLATION, DIMENSION,
    CoordZYX, STACK_ID, N5_PATH, VOLUME_DS
)
...
def find_and_show_best_boxes(all_cache, basin_cache, side_length):
    ...
    for cluster_id in largest[:50]:
        ...
        print(f'Cluster at offset {offset}px, shape {shape}px has {conn_count} connectors')
    ...
    ax.scatter(
        scatter_info['x'], scatter_info['y'], scatter_info['z'],
...
def judge_box(offset_nm, shape_nm, all_cache, basin_cache, padding_nm=None, catmaid=None, verbose=False):
    if catmaid is None:
        catmaid = get_catmaid()

    missing_slice_loc = {int(z_idx) * RESOLUTION.z + TRANSLATION.z for z_idx in catmaid.get_missing_sections(STACK_ID)}

    max_bound = offset_nm + shape_nm
    contained_missing = {z for z in missing_slice_loc if offset_nm.z <= z <= max_bound.z}
    ...
    if padding_nm:
        ...
        padded_judgement = judge_box(padded_offset, padded_shape, all_cache, basin_cache, catmaid=catmaid)
...
def judge_boxes(side_length_nm=3500, padding_nm=None):
    ...
    for name, center in [('VNC', vnc_loc_center), ('Brain', brain_loc_center)]:
        ...
        judgement = judge_box(offset, shape, all_cache, basin_cache, padding_nm=padding_nm, catmaid=catmaid)
        print(name)
