...
from clefts.constants import CONN_CACHE_PATH, DB_CREDENTIALS_PATH, BASIN_CACHE_PATH, BASIN_ANNOTATION, CoordZYX, PROJECT_ID
...
def get_caches(conn_path=CONN_CACHE_PATH, basin_path=BASIN_CACHE_PATH, force=False):
    ...
    if should_populate_basin:
        catmaid = get_catmaid()
        basin_skids = [row['skeleton_id'] for row in catmaid.get_skeletons_by_annotation(BASIN_ANNOTATION)]
...
class ConnectorCache:
    ...
    def __getitem__(self, item):
        row = self.cursor().execute('SELECT * FROM connector WHERE id = ? LIMIT 1', (item, )).fetchone()
    ...
    def _get_connection(self, recreate):
        ...
        if should_setup:
            ...
            cache_cursor.execute('CREATE TABLE connector (id INTEGER PRIMARY KEY, z REAL, y REAL, x REAL);')
    ...
    def populate_from(self, connection=conn, id_set=None, project_id=PROJECT_ID):
        ...
        if id_set is None:
            src_curs.execute(
                'SELECT id, location_z, location_y, location_x FROM connector WHERE project_id = %s;', (project_id,)
            )
        else:
            src_curs.execute('''
        ...
        with self:
            self.cursor().executemany(
                'INSERT OR IGNORE INTO connector (id, z, y, x) VALUES (?, ?, ?, ?)', src_curs.fetchall()
            )

    def _roi_to_params(self, offset, shape):
        upper = offset + shape
        return {
            'zmin': offset.z, 'zmax': upper.z,
            'ymin': offset.y, 'ymax': upper.y,
            'xmin': offset.x, 'xmax': upper.x,
        }
