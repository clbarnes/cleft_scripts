...
def validate_args(parsed_args):
    ...
    output_msg = (
        "For multiple ROIs, output must either be a str.format pattern with {} for indexing, "
...
def get_connectors_in_volume(catmaid, stack_id, roi_zyx_s):
    ...
    for connector_row in response[1]:
        ...
        if not in_roi(roi_zyx_p, zyx_p):
            continue
        z_idx, y_idx, x_idx = (
            coord_trans.project_to_stack_array(zyx_p, ORDER) - roi_zyx_s[0, :]
        ).astype(int)
...
def main(parsed_args):
    ...
    if parsed_args.threads:
        im_fetcher = ThreadedImageFetcher.from_catmaid(
            catmaid, threads=parsed_args.threads, **kwargs
        )
    else:
    ...
    for idx, (roi, path) in enumerate(zip(parsed_args.roi, parsed_args.output)):
        ...
        padded_roi = pad_roi(
            roi, [parsed_args.pad_z, parsed_args.pad_xy, parsed_args.pad_xy]
        )
        img_arr = im_fetcher.get_stack_space(padded_roi)

        connector_vol = (
            get_connectors_in_volume(catmaid, parsed_args.stack_id, padded_roi)
            if conns
            else None
        )
